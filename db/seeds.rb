...
5.times do
    users.each do |user|
        feed = user.feeds.create!(start_place:"hongkong",start_time:"2016-6-10",end_place:"auckland",end_time:"2016-6-29",feed_content:"has holiday")
        user.calendars.create!(feed_id: feed.id,calendar_type: Calendar::TYPE_OWNER, exact_time:"2016-6-11")
        id = user.id == 3? 1: (user.id+1)
